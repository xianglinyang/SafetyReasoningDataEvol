...
QUESTION_USER_PROMPT = """As a expert in analyzing harmful questions, your tasks are:
...
def format_scot_output_variable_two_stage(
    ...
    if not modified_summary.endswith(".") and not modified_summary.endswith("?"):
    ...
    stage1_block = f"{selected_summary_transition} {modified_summary}"
...
def adapt_scot_string_two_stage(input_scot_string: str, intention: str = None) -> str | None:
    ...
    try:
        ...
        if regulation_check_details.startswith("with following categories before generating the response.\n"):
            regulation_check_details = regulation_check_details.split("with following categories before generating the response.")[-1].strip()
        ...
        print(original_summary_sentence)
        print("Regulation Check Details:")
        print(regulation_check_details)
        print("Overall Conclusion Paragraph:")
        print(overall_conclusion_paragraph)
